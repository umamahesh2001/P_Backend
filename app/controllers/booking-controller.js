...
const Payment = require('../models/payment-model'); // Ensure Payment model is imported
...
bookingCntrl.list = async (req, res) => {
    ...
    try {
        const booking = await Booking.findById(id)
            ...
            .populate('vehicleId')
            .populate('customerId', 'name email mobileNumber'); // Useful for receipt
        ...
        bookingData.paymentDetails = payment;

        res.json(bookingData);
    } catch (err) {
        res.status(500).json({ error: "internal server error" }) // Changed to 500 for consistency
        console.log(err)
    }
}
    ...
    try {
        ...
        if (type === 'pending_approval') {
            query = {
                ...query,
                approveStatus: false,
                // $or: [{ status: 'pending' }, { approveStatus: false }],
                status: { $ne: 'cancelled' }
            };
        ...
        } else if (type === 'active') { // Includes upcoming and ongoing
            query = {
                ...query,
            ...
            query = {
                ...
                //     { status: 'completed' },
                //     {
                //         paymentStatus: { $in: ['paid', 'Successful', 'success'] },
                //         endDateTime: { $lt: now }
                //     }
                // ]
            };
        ...
        res.status(200).json(response); // Changed to 200 standard OK
    } catch (err) {
        console.log(err);
