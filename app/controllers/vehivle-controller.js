...
vehicleCtlr.listApprovalVehicles = async (req, res) => {
    try {
        const response = await Vehicle.find({ isVerified: false }).sort({ createdAt: -1 })
...
vehicleCtlr.update = async (req, res) => {
    ...
    const body = _.pick(req.body, ["vehicleName", "vehicleType", "vehicleNumber"])
